...
async function runTool(name, args, { config, conn, agent }) {
    if (name === "check_availability") {
        ...
        console.log(
            "[CALCOM] check_availability —",
            JSON.stringify({
                eventTypeId,
                dateFrom,
                dateTo,
                timeZone: args.timeZone || config.timeZone || "UTC",
                slotDateKeys: Object.keys(slots || {}),
                slotsSample: JSON.stringify(slots).slice(0, 600),
            }),
        );
...
// Returns the human-readable string the model should read back to the caller.
...
export async function runCalcomToolForAgent(agentId, name, args, expectedUserId) {
    ...
    if (!id || Number.isNaN(id)) return "I couldn't identify which agent to use.";

    const agent = await db.query.callAgents.findFirst({
        where: eq(callAgents.id, id),
        columns: { id: true, creatorId: true, name: true },
    });
    if (!agent) return "That agent could not be found.";
    if (expectedUserId && agent.creatorId !== expectedUserId) {
        return "You don't have access to this agent's calendar.";
    }

    const config = await loadAgentCalcomConfig(id);
    if (!config) return "Cal.com is not enabled for this agent.";

    const conn = await getConnection(agent.creatorId, "calcom");
    if (!conn?.apiKey) return "Cal.com is not connected for this account.";

    try {
        return await runTool(name, args || {}, { config, conn, agent });
    } catch (e) {
        console.error(`[CALCOM TOOL] ${name} failed:`, e?.message);
        return `Sorry, I ran into an issue: ${e?.message || "unknown error"}.`;
    }
}
