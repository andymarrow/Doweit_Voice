...
function formatSlotSummary(slotsByDate, max = 8) {
    ...
    for (const [date, slots] of Object.entries(slotsByDate)) {
        ...
        const times = (slots || [])
            .slice(0, 4)
            .map((s) => {
                // Cal.com v2 /slots returns slot objects keyed `start` (older
                // shapes used `time`); tolerate a bare ISO string too.
...
async function runTool(name, args, { config, conn, agent, debug }) {
    if (name === "check_availability") {
        ...
        const timeZone = args.timeZone || config.timeZone || "UTC";

        const slots = await findSlots(conn, { eventTypeId, dateFrom, dateTo, timeZone });

        const slotDateKeys = Object.keys(slots || {});
        const totalSlots = slotDateKeys.reduce(
            (n, k) => n + (Array.isArray(slots[k]) ? slots[k].length : 0),
            0,
        );
        const info = {
            eventTypeId,
            ...
            slotDateKeys,
            totalSlots,
            rawSlots: JSON.stringify(slots).slice(0, 800),
        };
        console.log("[CALCOM] check_availability —", JSON.stringify(info));
        if (debug) Object.assign(debug, info);
...
// Execute a single Cal.com tool for an agent — used by runtimes that don't
// speak Vapi's webhook envelope (e.g. the in-browser Gemini Live test panel).
// Returns { result, debug }: `result` is the string the model reads back;
// `debug` is diagnostic detail (event type id, slot counts, errors) surfaced
// in the HTTP response so it's visible in the browser console.
// Pass `expectedUserId` to enforce that the caller owns the agent.
export async function runCalcomToolForAgent(agentId, name, args, expectedUserId) {
    const debug = { tool: name };
    const id = typeof agentId === "string" ? parseInt(agentId, 10) : agentId;
    if (!id || Number.isNaN(id)) {
        return { result: "I couldn't identify which agent to use.", debug };
    }

    const agent = await db.query.callAgents.findFirst({
        where: eq(callAgents.id, id),
        columns: { id: true, creatorId: true, name: true },
    });
    if (!agent) return { result: "That agent could not be found.", debug };
    if (expectedUserId && agent.creatorId !== expectedUserId) {
        return { result: "You don't have access to this agent's calendar.", debug };
    }

    const config = await loadAgentCalcomConfig(id);
    if (!config) {
        debug.calcomEnabled = false;
        return { result: "Cal.com is not enabled for this agent.", debug };
    }
    debug.calcomEnabled = true;
    debug.eventTypeId = config.eventTypeId ?? null;
    debug.scope = config.scope ?? null;

    const conn = await getConnection(agent.creatorId, "calcom");
    if (!conn?.apiKey) {
        debug.connected = false;
        return { result: "Cal.com is not connected for this account.", debug };
    }
    debug.connected = true;
    debug.calBaseUrl = conn.baseUrl || "https://api.cal.com/v2";

    try {
        const result = await runTool(name, args || {}, { config, conn, agent, debug });
        return { result, debug };
    } catch (e) {
        console.error(`[CALCOM TOOL] ${name} failed:`, e?.message);
