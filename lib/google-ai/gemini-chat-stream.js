...
const apiKey = GEMINI_API_KEY; // Ensure GEMINI_API_KEY is in your environment variables
if (!apiKey) {
...
export async function getAiStreamResponse(
	...
	const charBehavior = Array.isArray(characterConfig.behavior)
		? characterConfig.behavior.join(", ")
		: "neutral";
	...
	try {
		...
		session = await ai.live.connect({
			...
			callbacks: {
				...
				onclose: (e /*: CloseEvent*/) => {
					...
					if (!turnIsResolved) {
						console.warn(
							"AI session closed before turn complete signal. Reason:",
							e.reason,
						);
		...
		if (session) {
			try {
				...
				console.error(
					"Error closing AI session after turn complete:",
					closeErr,
				);
		...
		if (session) {
			try {
				...
				console.error(
					"Error closing AI session after initial error:",
					closeErr,
				);
			}
		}
