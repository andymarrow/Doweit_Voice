...
export const callAgents = pgTable("call_agents", {
  ...
  marketplaceConfig: jsonb("marketplace_config")
    .default({ isForSale: false, usageCount: 0 })
    .notNull(),
...
export const interviews = pgTable("interviews", {
  id: serial("id").primaryKey(),
  agentId: integer("agent_id").references(() => callAgents.id, {
    onDelete: "cascade",
  }), // Made nullable for magic link interviews
  ...
  positionId: varchar("position_id", { length: 13 }).references(
    () => jobPositions.id,
    { onDelete: "set null" },
  ),
...
export const candidateApplications = pgTable("candidate_applications", {
  ...
  pass: boolean("pass"), // null until evaluated; true if total score >= position.passScore (stored in jobPositions.evaluationDescription), false otherwise
...
// --- NEW: Position Ratings (marketplace) ---
// One rating per user per public position. UI shows the user's current vote
// (if any) and the aggregated average. Enforced at the unique index level so
// repeated POSTs become idempotent UPSERTs.
export const positionRatings = pgTable(
  ...
  (t) => ({
    uniqUserPosition: uniqueIndex("position_ratings_user_position_uniq").on(
      t.positionId,
      t.userId,
    ),
  }),
...
export const positionRatingsRelations = relations(positionRatings, ({ one }) => ({
  position: one(jobPositions, {
    fields: [positionRatings.positionId],
    references: [jobPositions.id],
  }),
  user: one(users, {
    fields: [positionRatings.userId],
    references: [users.id],
  }),
}));
...
export const jobPositionsRelations = relations(
  ...
  ({ one, many }) => ({
    ...
    ratings: many(positionRatings),
  }),
);

export const interviewLinksRelations = relations(
  interviewLinks,
  ({ one, many }) => ({
    position: one(jobPositions, {
...
export const traineeProgressRelations = relations(
  traineeProgress,
  ({ one }) => ({
    user: one(users, {
...
export const traineeInterviewsRelations = relations(
  traineeInterviews,
  ({ one }) => ({
    user: one(users, {
...
export const traineeQuizAttemptsRelations = relations(
  traineeQuizAttempts,
  ({ one }) => ({
    user: one(users, {
...
export const sdkApps = pgTable("sdk_apps", {
  ...
  agentId: integer("agent_id").references(() => callAgents.id, {
    onDelete: "set null",
  }), // Links to the Alan AI agent's voice/prompt config
  ...
  customKnowledgeBaseId: integer("custom_knowledge_base_id").references(
    () => knowledgeBases.id,
    { onDelete: "set null" },
  ),
...
export const sdkManifests = pgTable("sdk_manifests", {
  ...
  appId: integer("app_id")
    .notNull()
    .references(() => sdkApps.id, { onDelete: "cascade" }),
...
export const agentIntegrationsRelations = relations(
  agentIntegrations,
  ({ one, many }) => ({
    agent: one(callAgents, {
...
export const integrationDispatchLogRelations = relations(
