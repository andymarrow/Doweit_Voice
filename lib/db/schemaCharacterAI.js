...
export const usersRelations = relations(users, ({ many }) => ({
	...
	characterLikes: many(characterLikes), // A user can like many char
	voices: many(voices), // A user can create many voices
	callAgents: many(callAgents),
    traineeProgress: many(traineeProgress), // Link to progress
...
export const interviews = pgTable("interviews", {
    ...
    agentId: integer("agent_id")
        .notNull()
        .references(() => callAgents.id, { onDelete: "cascade" }), // Link to Recruiter Agent
    ...
    type: varchar("type", { length: 50 }).notNull(), // 'real_application' | 'mock_training'
