...
export async function getAuthenticatedClient(userId) {
    ...
    // 1. Find the user's *Google* connection. Filtering by provider matters
    // because the same user may have other rows in this table (slack,
    // calcom, telegram, etc.) — picking the wrong one yields a token that
    ...
    const connection = await db.query.userConnections.findFirst({
        where: and(
            ...
            eq(userConnections.provider, 'google'),
        ),
    });
