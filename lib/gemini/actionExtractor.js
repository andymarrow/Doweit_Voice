// /lib/gemini/actionExtractor.js
import { GoogleGenerativeAI } from "@google/generative-ai";
import { db } from "@/lib/database";
import { callActionValues } from "@/lib/db/schemaCharacterAI";
import { dispatchIntegrationsForCall } from "@/lib/integrations/dispatcher";
	...
	const actionInstructions = actions
		.map((agentAction) => {
			...
			if (action.config?.type === "Choice") {
				const choices = action.config.options
					.map((o) => `"${o.label}"`)
					.join(", ");
...
export async function extractActionValuesFromTranscript(
	callId,
	agentActions,
	transcript,
) {
	...
	console.log(
		`[ANALYSIS] Gemini Raw Response for callId ${callId}:`,
		responseText,
	);
	...
	try {
		...
		console.error(
			`[ANALYSIS] Failed to parse JSON from Gemini for callId ${callId}. Response: ${jsonString}`,
		);
		throw new Error("Invalid JSON response from AI.");
	}

	console.log(
		`[ANALYSIS] Parsed extracted data for callId ${callId}:`,
		extractedData,
	);
	...
	if (valuesToInsert.length > 0) {
		console.log(
			`[ANALYSIS] Inserting ${valuesToInsert.length} action values (including nulls) into DB for callId ${callId}.`,
		);
		await db.insert(callActionValues).values(valuesToInsert);
		console.log(
			`[ANALYSIS] Successfully saved extracted values for callId ${callId}.`,
		);
	} else {
		console.log(
			`[ANALYSIS] No actions were found in the Gemini response for callId ${callId}. Nothing to save.`,
		);
	}

	// Fan out to per-agent integrations (Slack/Telegram/Email/...).
	// Fire-and-forget — a failing integration must never break extraction,
	// but we still await so any thrown errors get caught and logged here.
	try {
		await dispatchIntegrationsForCall(callId);
	} catch (err) {
		console.error(
			`[ANALYSIS] Integration dispatch failed for callId ${callId}:`,
			err?.message,
		);
	}
}
