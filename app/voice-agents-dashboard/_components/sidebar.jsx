		...
		>
			...
			<div className="flex items-center mb-6 p-2 space-x-3">
				...
				{isOpen ? (
					...
					>
						{" "}
						{/* Conditional margin for alignment */}
						...
						>
							<div className="p-2 text-sm">
								<Link href="/settings" legacyBehavior>
									<a className="block px-3 py-2 rounded hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
										{" "}
										{/* Subtle hover */}
										Settings
									</a>
			...
			{/* Tokens/Upgrade */}
			<div
				className={`mb-6 p-2 flex items-center ${isOpen ? "justify-start" : "justify-center"} text-gray-700 dark:text-gray-300`}
			>
				{" "}
				{/* Scheme text colors */}
				<AnimatePresence>
					{isOpen ? (
						...
						>
							...
							<Link href="/upgrade" legacyBehavior>
								...
								<a
									className={`bg-gradient-to-r from-cyan-400 to-cyan-600 hover:from-cyan-500 hover:to-cyan-700 text-white text-xs px-3 py-1 rounded-full font-semibold
				...
				{mainNavItems.map((item) => (
					<Link key={item.name} href={item.href} legacyBehavior>
						...
						>
							...
							<item.icon className={` ${isOpen ? "" : " text-xl"}`} />{" "}
							{/* Apply size class to icon */}
							<AnimatePresence>
								{isOpen && (
									<motion.span
										key={item.name}
										...
										exit="collapsed"
										variants={itemVariants}
										className={` ${isOpen ? "pl-2" : " "} whitespace-nowrap `}
									>
										{item.name}
									</motion.span>
								)}
			...
			<AnimatePresence>
				{isOpen && (
					<motion.h3
						...
						exit="collapsed"
						variants={itemVariants}
						className="text-xs uppercase font-semibold p-2
										...
										<AnimatePresence>
											{isOpen && (
												<motion.span
													...
													exit="collapsed"
													variants={itemVariants}
													className={` ${isOpen ? "pl-2" : ""} whitespace-nowrap flex-grow`}
												>
													{item.name}
												</motion.span>
											)}
									...
									{item.isOpen && isOpen && (
										...
										>
											{item.subItems.map((subItem) => (
												...
												>
													<a
														className="flex items-center p-2 pl-6 rounded-md transition-colors
                                                                hover:bg-gray-100 dark:hover:bg-gray-800
			...
			<div
				className={`mt-auto pt-4 border-t
                             ...
                             flex ${isOpen ? "justify-center items-center space-x-4" : "justify-center space-x-0 flex-col items-center space-y-4"}`}
			>
				{" "}
				{/* Scheme border and alignment */}
				{socialLinks.map((link) => (
					<Link key={link.name} href={link.href} legacyBehavior>
						...
						>
							{/* Icon color inherits from text color */}
							<link.icon className={isOpen ? "text-xl" : "text-2xl"} />{" "}
							{/* Applied size class to icon directly, increased collapsed size */}
							<AnimatePresence>
								{isOpen && (
									<motion.span
										key={link.name + "-text"}
