...
function TestAgentSidePanel({ isOpen, onClose, agent }) {
	...
	useEffect(() => {
		...
		if (!vapiRef.current) {
			if (!VAPI_PUBLIC_API_KEY) {
				console.error(
					"VAPI_PUBLIC_API_KEY is not set. Voice test will not work.",
				);
			} else {
				...
				const saveTranscript = async () => {
					...
					// Build callDetails object for backend
					const callDetails = {
						phoneNumber: phoneNumber || "", // <-- replace with your own state/prop
						...
						transcript: transcriptBuffer, // full transcript JSON
					};
				...
				vapiRef.current.on("message", (message) => {
					...
					if (message.type === "transcript") {
						...
						if (message.transcriptType === "final") {
							...
							const finalMsg = {
								id: date.toString() + "_" + senderRole,
								sender: senderRole,
								text: message.transcript,
								timestamp: date,
							};
							transcriptBuffer.push(finalMsg);
							console.log("Transcript appended:", finalMsg);
						}
				...
				vapiRef.current.on("error", (error) => {
					...
					setCallStatus("error");
					setCallError(error.message || "An unknown Vapi error occurred.");
					toast.error(`Vapi Error: ${error.message || "Unknown error"}`);
				});
			}
		}

		// Cleanup when unmounting/agentId changes
		return () => {
	...
	const mapVoiceProviderToVapi = (provider) => {
		switch (
			provider?.toLowerCase() // Use lowercase for robustness
		) {
			case "elevenlabs":
				return "11labs"; // Corrected from 'elevenlabs' to '11labs'
			case "azure":
				return "azure";
			...
			default:
				console.warn(
					`Unknown voice provider "${provider}". Falling back to '11labs'.`,
				);
	...
	const handleCallMe = useCallback(async () => {
		...
		// *** Initiate a new call ***
		console.log(
			`Initiating Vapi call for agent ${agentId} to ${phoneNumber} (${userName})`,
		);
...
--- END KNOWLEDGE BASE ---`;
			...
			console.log(
				"[Workflow Page] Starting Vapi call with agent config:",
				vapiAgentConfig,
			);
			...
			try {
				...
				setCallError(`Failed to start call: ${e.message}`);
	...
	const handleSendChatMessage = (e) => {
		...
		if (chatInput.trim()) {
			...
			console.log(
				`Simulating sending chat message "${chatInput}" to agent ${agentId}`,
			);
	...
	const isCallButtonDisabled =
		!userName || !agentId || !VAPI_PUBLIC_API_KEY || !vapiRef.current;
	const callConnected =
		...
		<div className="fixed inset-0 z-50 flex justify-end " onClick={onClose}>
			...
			>
				...
				>
					...
					{callStatus !== "idle" && (
						<span
							className={`text-xs font-medium ml-2 px-2 py-1 rounded-full ${
								callStatus === "in-progress"
									? `${uiColors.statusBadgeSuccessBg} ${uiColors.statusBadgeSuccessText}`
									: callStatus === "connecting"
										? `${uiColors.statusBadgeWarningBg} ${uiColors.statusBadgeWarningText}`
										: callStatus === "ended"
											? `${uiColors.textSecondary}` // Use secondary for ended status
											: callStatus === "error"
												? `${uiColors.statusBadgeDangerBg} ${uiColors.statusBadgeDangerText}`
						...
						>
							...
							{/* Capitalize status */}
							{callStatus === "connecting" && (
								<FiLoader className="inline-block ml-1 w-3 h-3 animate-spin" />
							)}
						</span>
					)}
					<button
					...
					>
						{" "}
						{/* Disable close while connecting */}
				...
				<div
					className={`flex border-b ${uiColors.borderPrimary} ${callStatus === "connecting" || callStatus === "in-progres" ? "opacity-50 pointer-events-none" : ""}`}
				>
					<button
						...
						disabled={
							callStatus === "connecting" || callStatus === "in-progress"
						} // Ensure disabled when call is active
					>
				...
				<div
					className={`flex-grow overflow-y-auto p-4 hide-scrollbar ${callStatus === "connecting" || callStatus === "in-progres" ? "opacity-50" : ""}`}
				>
					{!testMethod && (
						<div className={`text-center ${uiColors.textSecondary} pt-8`}>
							...
							{!VAPI_PUBLIC_API_KEY && (
								<p
									className={`mt-4 p-3 rounded-md ${uiColors.alertWarningBg} ${uiColors.alertWarningText} text-sm inline-block`}
								>
									<FiAlertTriangle className="inline-block mr-2 w-4 h-4" /> Vapi
									API Key not configured. Voice testing unavailable.
								</p>
							)}
						</div>
					)}
					...
					{testMethod === "voice" && (
						...
						<div
							className={`flex flex-col space-y-4 ${callStatus === "connecting" || callStatus === "in-progres" ? "pointer-events-none" : ""}`}
						>
							{!VAPI_PUBLIC_API_KEY && (
								<div
									className={`p-3 rounded-md ${uiColors.alertDangerBg} ${uiColors.alertDangerText} text-sm`}
								>
									<FiAlertTriangle className="inline-block mr-2 w-4 h-4" /> Vapi
									API Key not configured. Voice testing disabled.
								</div>
							)}
							{callError &&
								callStatus === "error" && ( // Display specific Vapi call error if status is 'error'
									<div
										className={`p-3 rounded-md ${uiColors.alertDangerBg} ${uiColors.alertDangerText} text-sm`}
									>
										<FiAlertTriangle className="inline-block mr-2 w-4 h-4" />{" "}
							...
							<div>
								<label
									htmlFor="userName"
									className={`block text-sm font-medium mb-1 ${uiColors.textSecondary}`}
								>
									Your Name
								</label>
								<div
									className={`flex items-center border rounded-md ${uiColors.borderPrimary} ${uiColors.bgSecondary}`}
								>
									<FiUser
									...
									<input
										...
										disabled={
											...
											callStatus === "in-progress" ||
											!VAPI_PUBLIC_API_KEY
										} // Disable input while call active or no Vapi key
							...
							<button
								...
								className={`w-full px-4 py-2 rounded-md font-semibold transition-colors text-sm ${
									isCallButtonDisabled
										...
										: callStatus === "in-progress"
											? `${uiColors.alertWarningBg} cursor-pointer`
											: `${uiColors.accentPrimaryGradient} cursor-pointer` // Use danger color for End Call
								} ${callStatus === "in-progress" ? uiColors.alertWarningText : "text-white"}`}
							>
								{/* Show loader inside button if connecting */}
								{callStatus === "connecting" && (
									<FiLoader className="inline-block mr-2 w-4 h-4 animate-spin" />
								)}
						...
						<div className="flex flex-col h-full">
							<div className="flex-grow overflow-y-auto space-y-3 pb-4">
								{chatMessages.length === 0 ? (
									<div
										className={`text-center text-sm ${uiColors.textSecondary}`}
									>
									...
									chatMessages.map(
										(
											msg,
											index, // Add index for key if no other unique ID
										) => (
											<div
							...
							<form
								...
								className={`flex items-center pt-4 border-t ${uiColors.borderPrimary}`}
							>
								<input
									type="text"
									value={chatInput}
