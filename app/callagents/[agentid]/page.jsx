...
import React, { useState, useEffect } from 'react'; // Keep state/effect for page-specific data like calls/actions metrics
import { motion } from 'framer-motion';
import { useParams } from 'next/navigation'; // Can still use useParams if needed, but agentId is in context
import { FiAlertTriangle, FiRefreshCcw, FiPhone, FiBarChart2, FiLoader } from 'react-icons/fi'; // Added FiLoader
...
export default function AgentDetailMainPage() {
    ...
    useEffect(() => {
        if (agentId) { // Ensure agentId is available
    ...
    }, [agentId]); // Re-run effect if the agentId changes

    // Use agent.name for display
            ...
            >
                ...
                <div className="flex items-center space-x-4 flex-shrink-0">
                     ...
                     <button
                         ...
                         onClick={() => { if(agentId) loadMetrics(); }} // Only load if agentId is available
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
                    {/* Type Filter */}
                     ...
                     {/* Date Range Filter */}
                     <div className="flex items-center space-x-1">
                          <span className={`text-sm ${uiColors.textSecondary}`}>Period:</span> {/* Changed label slightly */}
                         <select className={`form-select block w-fit text-sm rounded-md p-2 ${uiColors.bgSecondary} ${uiColors.textPrimary} ${uiColors.borderPrimary} border outline-none ${uiColors.ringAccentShade} focus:ring-1 transition-colors`}>
                              ...
                              {/* Left: Empty State / Chart (Placeholder) */}
                               {dashboardMetrics.callsData.total === 0 ? (
                                   <div className={`flex flex-col items-center justify-center h-48 ${uiColors.textPlaceholder}`}>
                                       ...
                                       <p className="text-sm text-center">{agentName} has no calls recorded yet.</p> {/* Use real agent name */}
                     ...
                     >
                         ...
                         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              ...
                              {dashboardMetrics.actionsData.total === 0 ? (
                                  <div className={`flex flex-col items-center justify-center h-48 ${uiColors.textPlaceholder}`}>
                                      <FiBarChart2 className="w-10 h-10 mb-4" />
                                      <p className="text-sm text-center">No actions collected for this agent yet.</p> {/* Adjusted message */}
                                  </div>
                              ) : (
                                   // Render Chart component here when data is available
                                   <div className={`flex flex-col items-center justify-center h-48 ${uiColors.textPlaceholder}`}>
                             ...
                             <div className="flex flex-col space-y-4">
                                  <div className="flex justify-between items-center">
                                       <span className={`text-sm font-medium ${uiColors.textSecondary}`}>Total actions collected</span> {/* Adjusted label */}
