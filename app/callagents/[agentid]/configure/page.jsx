...
export default function AgentConfigurePage() {
	// Renamed to avoid confusion with the layout
	...
	useEffect(() => {
		if (initialAgent) {
			...
			setConfigData({
				...
				customVocabulary: initialAgent.customVocabulary
					? [...initialAgent.customVocabulary]
				...
				voiceConfig: initialAgent.voiceConfig
					? { ...initialAgent.voiceConfig }
					: {}, // Ensure object and copy
				callConfig: initialAgent.callConfig
					? { ...initialAgent.callConfig }
					: {}, // Ensure object and copy
			});
	...
	const handleTabChange = (tabKey) => {
		...
		if (configTabKeys.includes(tabKey)) {
			...
			console.warn(
				`[Configure Page] Attempted to switch to unknown tab key: ${tabKey}`,
			);
		...
		>
			...
			>
				<h1 className={`text-2xl font-bold ${uiColors.textPrimary}`}>
					Configure Agent: {initialAgent.name}
				</h1>
