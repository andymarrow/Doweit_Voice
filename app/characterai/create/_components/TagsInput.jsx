	...
	return (
		<div className="space-y-2">
			...
			<p className={`text-sm ${uiColors.textPlaceholder}`}>
				{" "}
				{/* Adjusted font size */}
				Add keywords to help users find your character. Press Enter or comma to
				add.
			</p>
			...
			{tags.length > 0 && (
				<div className="flex flex-wrap gap-2 mb-2">
					{tags.map((tag, index) => (
						<span
							key={index}
							className={`inline-flex items-center text-xs font-medium px-2.5 py-0.5 rounded ${uiColors.accentSubtleBg} ${uiColors.textAccent}`}
						>
							{" "}
							{/* Adjusted text color class */}
							{tag}
							<button
