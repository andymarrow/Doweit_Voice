...
function VoiceModal({ isOpen, onClose, onVoiceSelect }) {
    ...
    useEffect(() => {
        if (isOpen) { // Conditional logic is inside the effect callback
            const fetchVoices = async () => {
                ...
                try {
                    ...
                    if (!response.ok) {
                         ...
                         throw new Error(`Failed to fetch voices: ${response.status} - ${errorBody}`);
                    }
     ...
     useEffect(() => {
         const handleClickOutside = (event) => {
             if (modalRef.current && !modalRef.current.contains(event.target) && isOpen) {
                 ...
                 const isBackgroundClick = event.target.classList.contains('bg-opacity-50'); // Assuming your background div has this class
