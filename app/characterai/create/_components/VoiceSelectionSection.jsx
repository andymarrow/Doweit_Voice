...
function VoiceSelectionSection({ selectedVoice, onChooseVoiceClick }) {
    ...
    const handlePlayPreview = () => {
        if (!previewAudioPlayer || !selectedVoice?.sampleAudioUrl) {
            console.warn("Cannot play preview: player not initialized or no sample URL.", selectedVoice);
            setIsPlayingPreview(false);
            return;
        }
    ...
    return (
        <div className="space-y-2">
            ...
            <div className={`p-3 rounded-md border ${uiColors.borderPrimary} ${uiColors.bgSecondary} flex items-center space-x-4 w-full sm:max-w-md`}>
                {selectedVoice ? (
                     <>
                         {/* Voice Avatar */}
                         <div className="flex-shrink-0 w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center overflow-hidden">
                             {/* Use actual voice avatar URL if available */}
                              {selectedVoice.avatarUrl ? (
                                   <Image src={selectedVoice.avatarUrl} alt={`${selectedVoice.name} Avatar`} width={40} height={40} className="rounded-full object-cover" />
                              ) : (
                                  // Fallback
                         ...
                         {/* Voice Details */}
                          <div className="flex-grow">
                              <div className={`font-semibold text-md ${uiColors.textPrimary}`}>{selectedVoice.name}</div> {/* Adjusted font size */}
                     ...
                     <span className={`${uiColors.textPlaceholder} text-md font-medium`}>No voice selected</span> 
                )}
            </div>
