...
import { FiX, FiPhoneOff, FiVolume2, FiMicOff, FiMic } from "react-icons/fi";
		...
		>
			...
			>
				...
				{callStatus === "ended" && (
					<button
						onClick={onClose}
						className={`absolute top-4 right-4 p-1 rounded-md ${uiColors.hoverBgSubtle}`}
						title="Close"
					>
				...
				<div className="mb-6">
					...
					{typeof characterAvatarUrl === "string" && characterAvatarUrl ? (
						...
						<div
							className={`w-20 h-20 rounded-full ${uiColors.bgSecondary} flex items-center justify-center text-3xl font-semibold mx-auto mb-4 text-gray-500 dark:text-gray-400 border ${uiColors.borderPrimary}`}
						>
							{characterName?.charAt(0).toUpperCase() || "?"}
						</div>
					)}
					<h3 className={`text-xl font-semibold ${uiColors.textPrimary}`}>
						{characterName}
					</h3>
				</div>
				...
				<div className="mb-8">
					{callStatus === "connecting" && (
						<div
							className={`text-sm font-medium ${uiColors.textSecondary} flex items-center justify-center`}
						>
						...
						<div className="mt-4 h-8 flex items-center justify-center space-x-1">
							{Array(5)
								...
								.map((_, i) => (
									<div
										key={i}
										className={`w-1 rounded-full ${uiColors.accentPrimaryGradient} animate-wave`}
										style={{
											height: `${(i + 1) * 10 + 20}%`,
				...
				<div
					className={`flex items-center justify-center space-x-6 ${callStatus !== "ongoing" ? "opacity-0 pointer-events-none" : ""} transition-opacity duration-300`}
				>
					{" "}
					{/* Added transition */}
					{/* Mute Toggle */}
