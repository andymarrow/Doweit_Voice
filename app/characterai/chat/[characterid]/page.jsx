...
import { VAPI_PUBLIC_KEY } from "@/configs/constants";
...
export default function SingleCharacterChatRoom() {
		...
		if (messagesEndRef.current) {
			...
			if (parent) {
				...
				const isScrolledToBottom =
					parent.scrollHeight - parent.clientHeight <= parent.scrollTop + 100; // Increased buffer slightly
	...
	return (
		<div className="flex flex-col h-full max-h-screen">
				...
				<div className="flex items-center min-w-0">
					...
					<div className="min-w-0">
						...
						{character.creatorName && (
							<p className={`text-xs ${uiColors.textSecondary} truncate`}>
					...
					>
						...
						{partialTranscripts.user && (
							...
							<p className="text-right text-wrap break-words">
								{user?.username || "You"}: {partialTranscripts.user}
							</p>
						)}
						...
						{partialTranscripts.character && (
							...
							<p className="text-left text-wrap break-words">
								{character?.name || "Character"}: {partialTranscripts.character}
							</p>
						)}
					</div>
				)}
			...
			<div
				className={`flex-shrink-0 p-4 border-t ${uiColors.borderPrimary} bg-white dark:bg-gray-900 w-full fixed bottom-0 `}
			>
				<div className={`text-center text-xs ${uiColors.textSecondary} mb-3`}>
					...
					fiction <FiAlertCircle className="inline-block ml-1 w-3 h-3" />
				</div>
				<form
