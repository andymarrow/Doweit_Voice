...
const lowerNavItems = [
	...
	{ name: "Friend", icon: FiUser, href: "/characterai?category=Friend" },
	{
		...
		href: "/characterai?category=Gamer",
		badge: "0%",
	},
];
...
const Sidebar = ({ isOpen, toggleSidebar }) => {
	...
	const fetchRecentCharacters = async () => {
		...
		try {
			...
			const response = await fetch(
				`/api/users/${user.id}/recent-characters`
			); // Example endpoint
			if (!response.ok) {
	...
	const handleProfileClick = () => {
		if (isOpen && !isUserLoading) {
			// Only toggle if open and user data is loaded
			setIsProfileDropdownOpen(!isProfileDropdownOpen);
		} else {
			console.log(
				"Profile click ignored: sidebar collapsed or user loading"
			);
		}
	};
	...
	return (
		<div
			className={`h-full flex flex-col p-4 transition-width duration-300 hide-scrollbar overflow-y-auto
                        ${uiColors.bgPrimary} ${uiColors.textPrimary}
                        ${isOpen ? "w-64" : "w-[70px]"}`}
		>
			{" "}
			{/* Explicit width in px for collapsed */}
			{/* Logo/Brand Area */}
			...
			<div className={`mb-4 ${isOpen ? "" : "flex justify-center"}`}>
				...
				<Link href="/characterai/create" legacyBehavior>
					<a
						className={`inline-flex items-center rounded-full font-semibold transition-colors shadow-sm whitespace-nowrap
					...
					>
						<FiPlus
							className={`${isOpen ? "mr-2 w-5 h-5" : "w-6 h-6"}`}
						/>
			...
			<nav className="mb-6">
				{topNavItems.map((item) => (
					<Link key={item.name} href={item.href} legacyBehavior>
						...
						>
							<item.icon
								className={`${isOpen ? "w-5 h-5 mr-3" : "w-6 h-6"}`}
							/>
			...
			<div
				key="search-input-outer"
				className={`mb-6 ${isOpen ? "" : "flex justify-center"}`}
			>
				<AnimatePresence initial={false}>
					{isOpen ? (
						// Keep the input if search is intended *within* the sidebar
						<motion.div
							key="search-input-open"
							initial="collapsed"
							animate="open"
							exit="collapsed"
							variants={itemVariants}
							className={`flex items-center rounded-md border ${uiColors.borderPrimary} ${uiColors.bgSecondary} w-full transition-colors focus-within:border-${uiColors.accentPrimaryText} focus-within:ring-1 ${uiColors.ringAccentShade}`}
						>
							<FiSearch
						...
						<motion.button
							...
							exit="collapsed"
							variants={itemVariants}
							className={`p-2 rounded-md ${uiColors.bgSecondary} ${uiColors.textSecondary} ${uiColors.hoverBgSubtle} ${uiColors.hoverText}`}
						>
			...
			<AnimatePresence>
				{isOpen && (
					...
					>
						...
						{isLoadingRecent ? (
							<div
								className={`text-xs ${uiColors.textSecondary}`}
							>
								Loading...
							</div>
						) : recentError ? (
							...
							recentCharacters.map((character) => (
								...
								>
									<a
										className={`flex items-center p-2 rounded-md transition-colors mb-1 ${uiColors.hoverBgSubtle} ${uiColors.hoverText} text-sm font-medium ${uiColors.textPrimary}`}
									>
										{character.avatarUrl ? ( // Use avatarUrl from backend
											...
											<div
												className={`w-8 h-8 rounded-full ${uiColors.bgSecondary} flex items-center justify-center text-sm font-semibold text-gray-500 dark:text-gray-400 border ${uiColors.borderPrimary} mr-3 flex-shrink-0`}
											>
												{character.name
													?.charAt(0)
													.toUpperCase() || "?"}
			...
			<AnimatePresence>
				{isOpen && (
					<motion.div
						...
						exit="collapsed"
						variants={itemVariants}
						className={`h-px ${uiColors.borderPrimary} mb-6`}
					></motion.div>
				)}
			</AnimatePresence>
							...
							<AnimatePresence initial={false}>
								{isOpen && (
									...
									>
										...
										{item.badge && (
											<span
												className={`ml-auto text-xs font-semibold px-2 py-0.5 rounded-full ${uiColors.accentBadgeBg} ${uiColors.textAccent} whitespace-nowrap`}
											>
												{item.badge}
											</span>
										)}
				...
				{isOpen && (
					...
					>
						...
						<Link href="/privacy" legacyBehavior>
							<a
								className={`${uiColors.hoverTextAccentContrast}`}
							>
								Privacy Policy
							</a>
						</Link>
			...
			<div
				key="upgrade-button-outer"
				className={`w-full ${isOpen ? "mb-6" : ""}`}
			>
				<AnimatePresence initial={false}>
					{isOpen ? (
						<motion.div
							key="upgrade-button-open"
							initial="collapsed"
							animate="open"
							exit="collapsed"
							variants={itemVariants}
						>
							...
							<Link href="/upgrade" legacyBehavior>
								<a
									className={`block text-center text-sm font-semibold px-4 py-2.5 rounded-md transition-all shadow whitespace-nowrap
						...
						<motion.button
							key="upgrade-button-collapsed"
							initial="collapsed"
							animate="open"
							exit="collapsed"
							variants={itemVariants}
							className={`block mx-auto p-3 rounded-md transition-colors ${uiColors.bgSecondary} ${uiColors.textAccent} ${uiColors.hoverBgSubtle}`}
						>
							✨
						</motion.button>
					)}
			...
			{!isUserLoading && user ? ( // Only render if user is loaded and exists
				<div
					className={`relative ${isOpen ? "" : "flex justify-center"} mt-auto pt-4 border-t ${uiColors.borderPrimary}`}
				>
					<button
					...
					>
						...
						<AnimatePresence initial={false}>
							{isOpen && (
								<motion.div
									key="dropdown-arrow-bottom"
									initial="collapsed"
									animate="open"
									exit="collapsed"
									variants={itemVariants}
									className={`${uiColors.textSecondary}`}
								>
					...
					<AnimatePresence initial={false}>
						{isProfileDropdownOpen &&
							isOpen && ( // Ensure sidebar is open for dropdown
								<motion.div
									initial="collapsed"
									animate="open"
									exit="collapsed"
								...
								>
									<div className="p-2 text-sm">
										...
										<Link href="/settings" legacyBehavior>
											<a
												className={`block px-3 py-2 rounded transition-colors ${uiColors.hoverBgSubtle}`}
											>
												Settings
											</a>
										</Link>
										...
										<div
											className={`flex items-center px-3 py-2 rounded transition-colors ${uiColors.hoverBgSubtle}`}
										>
				...
				<AnimatePresence>
					...
					{!isOpen && ( // Show placeholder when collapsed
						<div
							className={`mt-auto pt-4 border-t ${uiColors.borderPrimary} flex justify-center`}
						>
							<div
								className={`w-8 h-8 rounded-full ${uiColors.accentPrimaryGradient} flex items-center justify-center text-white text-sm font-semibold`}
							>
								U
							</div>
						</div>
