...
import {
    ...
    FiUser, FiBook, FiFilm, FiMusic, FiMap, FiHome, FiCpu, FiBookOpen
} from 'react-icons/fi';
...
function CharacterCard({ character, currentUserId, onLikeToggle }) {
    ...
    return (
        <motion.div
                     ...
                     {character.avatarUrl ? (
                         ...
                         />
                          ...
                          // Fallback for characters without avatars
                           <div className={`w-16 h-16 rounded-full ${uiColors.bgSecondary} flex items-center justify-center text-xl font-semibold text-gray-500 dark:text-gray-400 border ${uiColors.borderPrimary} flex-shrink-0`}>
                    ...
                    {/* Display Creator Name */}
                     {character.creatorName && (
                         <p className={`text-xs ${uiColors.textSecondary} mb-1`}>
                             By {character.creatorName} {/* Display creator's name */}
                         </p>
                     )}
                    {/* Behavior Tags */}
            ...
            <p className={`text-sm ${uiColors.textSecondary} mb-4 flex-grow overflow-hidden text-ellipsis max-h-20`}> {/* Added max-height */}
                {description}
            </p>
            ...
            <div className={`flex items-center justify-between mt-auto pt-3 border-t ${uiColors.borderPrimary}`}>
                 ...
                 <div className={`flex items-center text-xs ${uiColors.textSecondary} space-x-3`}>
                     ...
                     >
                         ...
                         <FiHeart className={`mr-1 ${isLiked ? uiColors.textAccent : uiColors.textSecondary}`} />
                         {character.likes || 0} {/* Display numeric likes, default to 0 */}
                     </button>
                     {/* Share Metric (Placeholder for now) */}
                      ...
                      >
                         <FiShare2 className="mr-1" /> {character.shares || 0} {/* Display numeric shares, default to 0 */}
                      </button>
                     {/* Chats Metric */}
                      <div className="flex items-center">
                         <FiMessageCircle className="mr-1" /> {character.chats || 0} {/* Display numeric chats, default to 0 */}
                      </div>
                 </div>
                {/* Chat Button */}
                <Link href={`/characterai/chat/${character.id}`} legacyBehavior>
                     <a className={`inline-flex items-center px-3 py-1.5 text-sm font-semibold rounded-md transition-colors ${uiColors.accentPrimaryGradient} text-white ${uiColors.hoverBgSubtle}`}>
