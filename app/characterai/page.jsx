...
// NOTE: The placeholder data 'allCharacters' is removed as we will fetch from the backend.
...
export default function CharacterAiPage() {
    ...
    const fetchCharacters = async (searchQuery = '') => {
        ...
        try {
            ...
            const response = await fetch(`/api/characters?search=${encodeURIComponent(searchQuery)}`);
            if (!response.ok) {
            ...
            >
                 ...
                 <div className="flex-grow">
                     <h2 className={`text-xl font-bold ${uiColors.textPrimary}`}>
                         {isUserLoading ? "Loading user..." : `Welcome back, ${user?.firstName || user?.username || 'Guest'}`}
                     </h2>
                 </div>
                              ...
                              <CharacterCard
                                ...
                                onLikeToggle={async (charId, isCurrentlyLiked) => {
                                    // Simulate API call for liking/unliking
                                     console.log(`Attempting to ${isCurrentlyLiked ? 'unlike' : 'like'} character ${charId}`);
                                     try {
                                         ...
                                         // Update character list state optimistically or by refetching
                                          ...
                                          setCharacters(prevChars => prevChars.map(char => {
                                              if (char.id === charId) {
                                                  ...
                                                  const updatedLikes = isCurrentlyLiked ? char.likes - 1 : char.likes + 1;
