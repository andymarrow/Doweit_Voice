...
export async function GET(req, { params }) {
    ...
    try {
        ...
        const recentMessageCharacterIds = await db
         ...
         const uniqueCharacterIds = [...new Set(recentMessageCharacterIds.map(msg => msg.characterId))].slice(0, RECENT_CHARACTER_LIMIT);
        ...
        const recentCharacters = await db.query.characters.findMany({
            ...
            columns: { // Select only the fields needed by the sidebar component
                id: true,
                name: true,
                avatarUrl: true,
        ...
        console.log(`Returning ${sortedRecentCharacters.length} recent characters for user ${currentUserId}`);


        // Return the list of recent characters
        return NextResponse.json({ recentCharacters: sortedRecentCharacters }, { status: 200 });

    } catch (error) {
        console.error(`API Error /api/users/${paramUserId}/recent-characters (GET):`, error);
        return NextResponse.json({ error: 'Internal server error fetching recent characters' }, { status: 500 });
    }
}
