...
import { eq, desc, and } from "drizzle-orm"; // Import necessary Drizzle functions
...
export async function POST(req) {
	...
	try {
		...
		if (!details || typeof details !== "object") {
			return NextResponse.json(
				{ error: "Action configuration details are required." },
				{ status: 400 },
			);
		}
		if (!details.type) {
			return NextResponse.json(
				{
					error:
						"Action detail type (Text, Boolean, Choice, Action) is required in details configuration.",
				},
				{ status: 400 },
			);
		}
		...
		if (details.type === "Boolean") {
			if (
				details.trueLabel === undefined ||
				details.falseLabel === undefined ||
				details.trueLabel.trim() === "" ||
				details.falseLabel.trim() === ""
			) {
				return NextResponse.json(
					{
						error:
							"Boolean config requires non-empty trueLabel and falseLabel.",
					},
					{ status: 400 },
				);
			}
			...
			if (
				...
				return NextResponse.json(
					{
						error:
							"Choice config requires a non-empty array of options with non-empty labels.",
					},
					{ status: 400 },
				);
			}
		...
		if (existingAction) {
			console.warn(
				`[API POST /api/actions] Conflict: Action name "${name.trim()}" already exists for user ${userId}.`,
			);
			return NextResponse.json(
				{ error: `An action with the name "${name.trim()}" already exists.` },
				{ status: 409 },
			); // 409 Conflict
		}
		...
		if (!newActions || newActions.length === 0) {
			console.error(
				`[API POST /api/actions] DB Insert failed for user ${userId}. Insert result:`,
				newActions,
			);
			return NextResponse.json(
				{ error: "Failed to create action in database" },
				{ status: 500 },
			);
		}
		...
		console.log(
			`[API POST /api/actions] Action created successfully for user ${userId}. ID: ${newAction.id}`,
		);
		...
		if (error.message.includes("action_creator_name_unq")) {
			...
			console.warn(
				`[API POST /api/actions] Likely unique constraint violation for user ${userId} and name "${body?.name}".`,
			);
			return NextResponse.json(
				{ error: `An action with the name "${body?.name}" already exists.` },
				{ status: 409 },
			); // 409 Conflict
		}
		return NextResponse.json(
