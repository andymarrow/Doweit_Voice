...
export async function PUT(req, { params }) {
	...
	if (!actionId) {
		console.warn(
			`[API PUT /actions/${actionId}] Bad Request: Invalid action ID format.`,
		);
	...
	try {
		...
		if (updateData.config !== undefined) {
			...
			if (!updateData.config.type) {
				return NextResponse.json(
					{ error: "Action detail type is required within config" },
					{ status: 400 },
				);
			}
			...
			if (configType === "Boolean") {
				...
				if (
					...
					return NextResponse.json(
						{
							error:
								'"True" and "False" labels are required and cannot be empty for Boolean type',
						},
						{ status: 400 },
					);
				}
				...
				if (
					...
					return NextResponse.json(
						{
							error:
								"Choice config requires a non-empty array of options, each with a non-empty string label",
						},
						{ status: 400 },
					);
				}
			...
			// Add validation for 'Action' type config if it has required fields (e.g., config.number for transfer)
				...
				if (
					updateData.config.config?.number !== undefined &&
					typeof updateData.config.config.number !== "string"
				) {
					return NextResponse.json(
						{ error: "Transfer number must be a string" },
						{ status: 400 },
					);
				}
		...
		if (!existingAction) {
			...
			console.warn(
				`[API PUT /actions/${actionId}] Not Found: Action ID ${actionId} not found or not owned by user ${userId}.`,
			);
			return NextResponse.json(
				{
		...
		if (
			...
			updateData.name.trim() !== existingAction.name
		) {
			console.log(
				`[API PUT /actions/${actionId}] Checking for name conflict: "${updateData.name.trim()}" for user ${userId}`,
			);
			...
			if (conflictingAction) {
				console.warn(
					`[API PUT /actions/${actionId}] Conflict: Action with name "${updateData.name.trim()}" already exists (ID: ${conflictingAction.id}).`,
				);
				return NextResponse.json(
					{
						error: `An action with the name "${updateData.name.trim()}" already exists.`,
					},
					{ status: 409 },
				); // 409 Conflict
			}
			...
			console.log(
				`[API PUT /actions/${actionId}] Name is not changing or is the same (was "${existingAction.name}", is "${updateData.name.trim()}"). Skipping conflict check.`,
			);
		} else {
			console.log(
				`[API PUT /actions/${actionId}] Name is not included in update data. Skipping conflict check.`,
			);
		}
		...
		if (Object.keys(updateData).length === 0) {
			console.log(
				`[API PUT /actions/${actionId}] No updateable fields provided in body.`,
			);
		...
		console.log(
			`[API PUT /actions/${actionId}] Updating action ID ${actionId} for user ${userId} with data:`,
			updateData,
		);
		...
		if (!updatedActions || updatedActions.length === 0) {
			...
			console.error(
				`[API PUT /actions/${actionId}] DB Update failed despite action found. Result:`,
				updatedActions,
			);
			return NextResponse.json(
				{ error: "Failed to update action in database" },
				{ status: 500 },
			);
		}
		...
		console.log(
			`[API PUT /actions/${actionId}] Action ID ${actionId} updated successfully. Returned data:`,
			updatedAction,
		);
...
export async function DELETE(req, { params }) {
	...
	if (!actionId) {
		console.warn(
			`[API DELETE /actions/${actionId}] Bad Request: Invalid action ID format.`,
		);
		return NextResponse.json({ error: "Invalid action ID" }, { status: 400 });
	}
	...
	try {
		console.log(
			`[API DELETE /actions/${actionId}] Attempting to delete action ID ${actionId} for user ${userId}.`,
		);
		...
		if (!deletedActions || deletedActions.length === 0) {
			...
			console.warn(
				`[API DELETE /actions/${actionId}] Not Found: Action ID ${actionId} not found or not owned by user ${userId}.`,
			);
			return NextResponse.json(
				{
		...
		console.log(
			`[API DELETE /actions/${actionId}] Action deleted successfully. Deleted ID: ${deletedActions[0].id}`,
		);

		// Return success response
		return NextResponse.json(
			{ success: true, deletedId: deletedActions[0].id },
			{ status: 200 },
		);
	} catch (error) {
