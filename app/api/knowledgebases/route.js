...
export async function GET() {
	...
	try {
		console.log(
			`[API KB LIST GET] Fetching knowledge bases for user ${userId}`,
		);
		...
		console.log(
			`[API KB LIST GET] Found ${myKnowledgeBases.length} knowledge bases for user ${userId}.`,
		);

		// Return the list of owned KBs
		// Add isOwner: true to each object for consistency, although they are all owned here
		const responseData = myKnowledgeBases.map((kb) => ({
			...kb,
			isOwner: true,
		}));
...
export async function POST(req) {
	...
	try {
		...
		if (!name || typeof name !== "string" || !name.trim()) {
			console.warn(
				"[API KB CREATE POST] Validation failed: Missing or invalid name.",
			);
			return NextResponse.json(
		...
		for (const field in body) {
			if (ALLOWED_KB_CREATE_FIELDS.includes(field) && field !== "name") {
				// Skip name as it's trimmed and handled
		...
		if (
			ALLOWED_KB_CREATE_FIELDS.includes("isPublic") &&
			body.isPublic === undefined
		) {
			finalInsertData.isPublic = false;
		}
		if (
			ALLOWED_KB_CREATE_FIELDS.includes("content") &&
			body.content === undefined
		) {
			finalInsertData.content = [];
		}
		if (
			ALLOWED_KB_CREATE_FIELDS.includes("status") &&
		...
		if (existingKb) {
			console.warn(
				`[API KB CREATE POST] Conflict: KB name "${finalInsertData.name}" already exists for user ${userId}.`,
			);
			return NextResponse.json(
				{
					error: `A Knowledge Base with the name "${finalInsertData.name}" already exists.`,
				},
				{ status: 409 },
			); // 409 Conflict
		}
		...
		if (!newKbs || newKbs.length === 0) {
			console.error(
				`[API KB CREATE POST] DB Insert failed for user ${userId}. Insert result:`,
				newKbs,
			);
			return NextResponse.json(
				{ error: "Failed to create Knowledge Base in database" },
				{ status: 500 },
			);
		}
		...
		console.log(
			`[API KB CREATE POST] Knowledge Base created successfully for user ${userId}. ID: ${newKb.id}`,
		);
		...
		if (error.message.includes("knowledge_bases_creator_id_name_unq")) {
			// Check for the unique index name
			console.warn(
				`[API KB CREATE POST] Likely unique constraint violation for user ${userId} and name "${body?.name}".`,
			);
			return NextResponse.json(
				{
					error: `A Knowledge Base with the name "${body?.name}" already exists.`,
				},
				{ status: 409 },
			); // 409 Conflict
		}
