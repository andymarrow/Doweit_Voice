...
export async function GET() {
	...
	try {
		console.log(
			`[API KB TEMPLATES GET] Fetching public knowledge bases for user ${userId}`,
		);
		...
		console.log(
			`[API KB TEMPLATES GET] Found ${publicKnowledgeBases.length} public knowledge bases.`,
		);
		...
		const responseData = publicKnowledgeBases.map((kb) => ({
			...
			author: kb.creator
				? `${kb.creator.firstName || ""} ${kb.creator.lastName || ""}`.trim() ||
					"System"
				: "System", // Default to 'System' if no creator or creator is null
