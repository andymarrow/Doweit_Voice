...
export async function GET(req, { params }) {
	...
	try {
		...
		if (!knowledgeBase) {
			console.warn(
				`[API KB DETAIL GET] KB ${kbId} not found or not authorized for user ${userId}.`,
			);
		...
		console.log(
			`[API KB DETAIL GET] Successfully fetched KB ${kbId}. Is owner: ${isOwner}.`,
		);
		...
		return NextResponse.json(
			{ ...knowledgeBase, isOwner, creator: undefined },
			{ status: 200 },
		); // Optionally remove creator object
	} catch (error) {
		console.error(
			`[API KB DETAIL GET] Database error fetching KB ${kbId} for user ${userId}:`,
			error,
		);
		return NextResponse.json(
...
export async function PATCH(req, { params }) {
	...
	try {
		...
		console.log(
			`[API KB UPDATE PATCH] Received update request for KB ${kbId} (user ${userId}). Updates:`,
			updates,
		);
		...
		if (!knowledgeBase) {
			console.warn(
				`[API KB UPDATE PATCH] KB ${kbId} not found or not owned by user ${userId}. Update denied.`,
			);
			return NextResponse.json(
				{ error: "Knowledge Base not found or not authorized" },
				{ status: 404 },
			); // Return 404 if not found/owned
		}
		...
		for (const field in updates) {
			if (ALLOWED_KB_UPDATE_FIELDS.includes(field)) {
				...
				if (field === "content" && !Array.isArray(updates[field])) {
					console.warn(
						`[API KB UPDATE PATCH] Invalid type for content: expected array`,
					);
					return NextResponse.json(
						{ error: `Invalid data format for field "${field}"` },
						{ status: 400 },
					);
				}
				if (field === "isPublic" && typeof updates[field] !== "boolean") {
					console.warn(
						`[API KB UPDATE PATCH] Invalid type for isPublic: expected boolean`,
					);
					return NextResponse.json(
						{ error: `Invalid data format for field "${field}"` },
						{ status: 400 },
				...
				if (
					...
					console.warn(
						`[API KB UPDATE PATCH] Invalid type for name: expected non-empty string`,
					);
					return NextResponse.json(
				...
				console.warn(
					`[API KB UPDATE PATCH] Attempted to update disallowed field: "${field}".`,
				);
		...
		if (Object.keys(updateData).length === 0) {
			console.warn(
				`[API KB UPDATE PATCH] No valid update fields provided for KB ${kbId}`,
			);
			return NextResponse.json(
		...
		if (!updatedKbs || updatedKbs.length === 0) {
			console.error(
				`[API KB UPDATE PATCH] Update failed for KB ${kbId}. DB result:`,
				updatedKbs,
			);
			return NextResponse.json(
		...
		console.log(
			`[API KB UPDATE PATCH] Knowledge Base ${kbId} updated successfully.`,
		);
		...
		console.error(
			`[API KB UPDATE PATCH] Unexpected API Error during KB update for KB ${kbId} (user ${userId}):`,
			error,
		);
		// Check for unique constraint violation if name was updated
		if (error.message.includes("knowledge_bases_creator_id_name_unq")) {
			console.warn(
				`[API KB UPDATE PATCH] Likely unique constraint violation on name update for user ${userId}.`,
			);
			// Find the attempted new name from updates
			const attemptedName = updates?.name;
			return NextResponse.json(
				{
					error: `A Knowledge Base with the name "${attemptedName}" already exists.`,
				},
				{ status: 409 },
			); // 409 Conflict
		}
...
export async function DELETE(req, { params }) {
	...
	try {
		console.log(
			`[API KB DELETE] Attempting to delete KB ${kbId} for user ${userId}`,
		);
		...
		if (!knowledgeBase) {
			console.warn(
				`[API KB DELETE] KB ${kbId} not found or not owned by user ${userId}. Deletion denied.`,
			);
			return NextResponse.json(
				{ error: "Knowledge Base not found or not authorized" },
				{ status: 404 },
		...
		console.log(
			`[API KB DELETE] Delete query executed for KB ${kbId}. Rows affected: ${rowsAffected}`,
		);
		...
		if (rowsAffected === 1) {
			...
			console.warn(
				`[API KB DELETE] KB ${kbId} not found after verification? Or already deleted concurrently? Rows affected: ${rowsAffected}.`,
			);
			return NextResponse.json(
				{ error: "Knowledge Base not found or already deleted" },
				{ status: 404 },
			);
		} else {
			console.error(
				`[API KB DELETE] Unexpected number of rows affected (${rowsAffected}) after successful verification for KB ${kbId}. Result object:`,
				result,
			);
			return NextResponse.json(
		...
		console.error(
			`[API KB DELETE] Database error deleting KB ${kbId} for user ${userId}:`,
			error,
		);
		return NextResponse.json(
			{ error: "Internal server error" },
