...
let bucket;
try {
    if (admin && admin.apps && admin.apps.length) { // Check if admin app and apps array are available/initialized
        bucket = getStorage(admin.apps[0]).bucket(); // Get bucket from the default app
         console.log("Firebase Storage bucket accessed.");
    } else {
         console.error("Firebase Admin not initialized. Audio uploads will likely fail.");
    }
} catch (e) {
        ...
        if (msg.sender === 'user' || msg.sender === 'character') {
            ...
            const role = msg.sender === 'user' ? 'user' : 'model';
             ...
             if (geminiContents.length > 0 && geminiContents[geminiContents.length - 1].role === role) {
                 console.warn(`Gemini history formatting: Found consecutive messages with the same role ('${role}'). Ensure history alternates 'user' and 'character'. History part:`, msg);
            ...
            geminiContents.push({
             ...
             console.warn(`Skipping message with unknown sender type in history: ${msg.sender}`);
        }
    }
...
export async function POST(req, { params }) {
    ...
    try {
        ...
        if (!text || typeof text !== 'string' || !Array.isArray(history)) {
            console.error("Invalid payload: message text (string) or history (array) missing/invalid.");
            return NextResponse.json({ error: 'Message text (string) and history (array) are required' }, { status: 400 });
        }
        ...
        if (!character) {
         ...
         if (!character.voiceProvider || !character.voiceId || !character.language) {
             console.error(`Character ${character.id} is missing voice configuration (provider: ${character.voiceProvider}, id: ${character.voiceId}, language: ${character.language}).`);
             return NextResponse.json({ error: 'Character voice configuration is incomplete in the database' }, { status: 500 });
         }


        // 3. Construct the LLM Prompt for Gemini
        if (!geminiAi) {
             return NextResponse.json({ error: 'LLM (Gemini) not initialized on server. Check GEMNI_API_KEY.' }, { status: 500 });
        }
        ...
        let generatedText = '';
        try {
             const model = geminiAi.getGenerativeModel({ model: 'gemini-1.5-flash' }); // Using the flash model ID directly
         ...
         if (!generatedText && generatedText !== '') { // Check for null, undefined, or empty string
             ...
             }, { status: 200 }); // Still return 200 for a valid process flow, just an empty/default response
         }
        ...
        try {
             ...
             if (!character.voiceProvider || !character.voiceId || !character.language) {
                 ...
                 if (!ttsResponse.ok) {
                     ...
                     console.error(`Vapi TTS API Error. Status: ${ttsResponse.status}, Body: ${errorBody}`);
                     // Log error but proceed without audio
                      audioBuffer = null; // Ensure no buffer to upload
                 } else {
                      console.log("Received successful response from Vapi TTS. Status:", ttsResponse.status);
                      ...
                      console.log(`Received audio data buffer of size ${audioBuffer?.byteLength} bytes.`);

                      if (!audioBuffer || audioBuffer.byteLength === 0) {
                          console.warn("Vapi TTS returned success but audio buffer is empty.");
                          audioBuffer = null;
                      }
                 }
        ...
        if (audioBuffer && bucket && firebaseProjectId && currentUserId && charIdInt) { // Added charIdInt check
            try {
                ...
                console.error("Error uploading audio to Firebase Storage:", firebaseUploadError);
                audioUrl = null; // Ensure audioUrl is null on failure
            }
        } else if (!audioBuffer) {
             console.log("No audio buffer to upload.");
        } else {
             console.warn("Firebase Storage bucket or required info not available or missing IDs for upload.");
             audioUrl = null;
        }
    ...
    [savedAiMessage] = await db.insert(chatMessages).values(aiMessageToSave).returning({
         id: chatMessages.id,
    ...
    // Log DB error. Proceed to return AI message if available, but warn.
     if (savedAiMessage) {
          console.warn("Messages generated but failed to save to DB. AI message still returned to frontend.");
     } else {
        ...
        const finalAiMessage = {
             ...
             sender: 'character',
             timestamp: savedAiMessage?.timestamp?.toISOString() || new Date().toISOString(),
        };
        ...
        console.error("API Error /api/chat/[characterId]/message (POST) - Top Level Catch:", error);
        // Catch any errors not handled above and return a generic 500
        return NextResponse.json({ error: 'Internal server error during chat message processing' }, { status: 500 });
    }
}