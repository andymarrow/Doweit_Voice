...
export async function POST(req, { params }) {
	try {
		...
		if (messages.length === 0) {
			console.log(
				`No messages to save for user ${userId} and character ${characterId}.`,
			);
			return NextResponse.json(
		...
		if (messagesToInsert.length === 0) {
			console.log(
				`No non-system messages to save for user ${userId} and character ${characterId}.`,
			);
			return NextResponse.json(
				{ message: "No non-system messages to save" },
				{ status: 200 },
			);
		}
		...
		return NextResponse.json(
			{ error: "Failed to save call history", details: error.message },
			{ status: 500 },
		);
	}
}
