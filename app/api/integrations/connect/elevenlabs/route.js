...
export async function POST(req) {
    try {
        ...
        return NextResponse.json({ success: true, message: 'ElevenLabs connected and voices synced.' }, { status: 200 });
...
async function syncElevenLabsVoices(userId, elevenlabsClient) {
    ...
    await db.delete(voices).where(and(
        eq(voices.creatorId, userId),
        eq(voices.provider, 'elevenlabs')
    ));
    
    await db.insert(voices).values(voicesToInsert);
    
    console.log(`Successfully synced ${voicesToInsert.length} voices for user ${userId}.`);
}