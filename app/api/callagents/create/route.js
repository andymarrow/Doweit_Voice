	...
	try {
		...
		if (agentImageFile instanceof File) {
			try {
				...
				console.log(
					`[API CREATE] Image uploaded successfully for user ${userId}: ${avatarUrl}`,
				);
			} catch (uploadError) {
				console.error(
		...
		if (!newAgents || newAgents.length === 0) {
			console.error(
				`[API CREATE] DB Insert failed for user ${userId}. Insert result:`,
				newAgents,
			);
			return NextResponse.json(
...
export async function GET() {
	...
	const userId = user?.id;
	console.log(
		`[API GET] Attempting to fetch agents. Current userId from auth: ${userId}`,
	);
	...
	try {
		...
		console.log(
			`[API GET] Drizzle query executed for user ${userId}. Found ${agents.length} agents.`,
		);
