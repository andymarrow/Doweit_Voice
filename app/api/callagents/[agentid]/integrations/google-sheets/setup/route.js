...
async function exportExistingAnalyzedCalls(userId, agentId, spreadsheetId) {
    console.log(`[G-Sheets Background Job] Checking for pre-analyzed calls to export for agent ${agentId}`);

    // Find calls for this agent that have action values but have not been exported yet.
    const callsToExport = await db.selectDistinct({ id: calls.id })
        .from(calls)
    ...
    if (callIds.length === 0) {
        console.log(`[G-Sheets Background Job] No pre-analyzed calls found to export.`);
        return;
    }

    console.log(`[G-Sheets Background Job] Found ${callIds.length} calls to export.`);
    ...
    try {
        ...
        console.log(`[G-Sheets Background Job] Successfully exported ${callIds.length} pre-analyzed calls.`);
    } catch (error) {
        console.error(`[G-Sheets Background Job] Failed to export pre-analyzed calls for agent ${agentId}:`, error);
...
export async function POST(req, { params }) {
    ...
    try {
        ...
        console.log(`[G-Sheets Setup] Creating new spreadsheet for agent: ${agent.name}`);
        ...
        await db.update(callAgents).set({
            integrationConfig: { ...agent.integrationConfig, googleSheets: newSheetData },
            updatedAt: new Date()
        }).where(eq(callAgents.id, agentId));

        console.log(`[G-Sheets Setup] Linked sheet ${newSheetData.spreadsheetId} for agent ${agentId}.`);
        ...
        if (error.message.includes("connection not found")) {
            return NextResponse.json({ error: "Google connection not found." }, { status: 400 });
        }
