...
export async function GET(req, { params }) {
	const client = new VapiClient({ token: VAPI_SECRET_API_KEY });
...
export async function DELETE(req, { params }) {
	...
	if (!userId) {
		console.log(
			`[API CALL DELETE] Unauthorized: No userId for agent ${agentId}, call ${callId}`,
		);
	...
	try {
		console.log(
			`[API CALL DELETE] Attempting to delete call ${callId} for agent ${agentId} (user ${userId})`,
		);
		...
		if (callToDelete.agentId !== agentId) {
			console.warn(
				`[API CALL DELETE] Call ${callId} belongs to agent ${callToDelete.agentId}, not expected agent ${agentId}.`,
			);
		...
		if (callToDelete.agent.creatorId !== userId) {
			console.warn(
				`[API CALL DELETE] Call ${callId} belongs to agent ${agentId}, which is not owned by user ${userId}. Unauthorized deletion attempt.`,
			);
			return NextResponse.json(
		...
		console.log(
			`[API CALL DELETE] Verification successful for call ${callId}. Proceeding with deletion.`,
		);
		...
		const result = await db.delete(calls).where(eq(calls.id, callId));
		...
		console.log(
			`[API CALL DELETE] Delete query executed for call ${callId}. Rows affected: ${rowsAffected}`,
		);
		...
		if (rowsAffected === 1) {
			...
			return NextResponse.json(
				{ success: true, deletedId: callId },
				{ status: 200 },
			);
			// return new NextResponse(null, { status: 204 }); // 204 No Content is also appropriate
		} else if (rowsAffected === 0) {
			console.warn(
				`[API CALL DELETE] Call ${callId} not found after verification? Or already deleted concurrently? Rows affected: ${rowsAffected}.`,
			);
			return NextResponse.json(
				{ error: "Call not found or already deleted" },
				{ status: 404 },
			);
		} else {
			console.error(
				`[API CALL DELETE] Unexpected number of rows affected (${rowsAffected}) after successful verification for call ${callId}. Result object:`,
				result,
			);
			return NextResponse.json(
		...
		console.error(
			`[API CALL DELETE] Database error deleting call ${callId} for agent ${agentId} (user ${userId}):`,
			error,
		);
		return NextResponse.json(
