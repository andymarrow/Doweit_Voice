...
export async function POST(req, { params }) {
	...
	try {
		console.log(
			`[ANALYSIS TRIGGER] Received request to analyze call ${callId} for user ${userId}.`,
		);
		...
		if (callRecord.agent.creatorId !== userId) {
			return NextResponse.json(
				{ error: "Not authorized to analyze this call" },
				{ status: 403 },
			);
		}
		...
		if (
			...
			callRecord.agent.agentActions.length === 0
		) {
			return NextResponse.json(
				{ message: "No actions configured for this agent, skipping analysis." },
				{ status: 200 },
			);
		}
		...
		return NextResponse.json(
			{ success: true, message: "Analysis has been initiated." },
			{ status: 202 },
		); // 202 Accepted is a good status code here
	} catch (error) {
