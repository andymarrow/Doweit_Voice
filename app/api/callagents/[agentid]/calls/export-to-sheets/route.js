...
export async function POST(req, { params }) {
    ...
    try {
        ...
        if (!Array.isArray(callIds) || callIds.length === 0) {
            return NextResponse.json({ error: 'An array of callIds is required.' }, { status: 400 });
        }
        ...
        if (!spreadsheetId) {
            return NextResponse.json({ error: 'Google Sheets integration is not configured for this agent.' }, { status: 400 });
        }

        // 2. Fetch and format the data using our export engine
        console.log(`[G-Sheets Export] Preparing to export ${callIds.length} calls for agent ${agentId}.`);
        const { headers, rows } = await prepareCallDataForExport(callIds);
        
        if (rows.length === 0) {
            return NextResponse.json({ message: 'No valid call data found for the provided IDs.' });
        }
        ...
        console.log(`[G-Sheets Export] Successfully exported ${rows.length} calls.`);
