...
import { callAgents, calls, callActionValues, agentActions, actions } from '@/lib/db/schemaCharacterAI';
...
export async function GET(req, { params }) {
    ...
    try {
        console.log(`[API CALLS GET] Fetching calls for agent ${agentId} (user ${userId})`);
        ...
        if (!agent) {
            console.warn(`[API CALLS GET] Agent ${agentId} not found or not owned by user ${userId}`);
        ...
        const agentCalls = await db.query.calls.findMany({
            ...
            with: {
                callActionValues: {
                    with: { // Join agentActions and actions
                        agentAction: {
                             with: { // Join the global action definition
                                action: true
                            }
                        }
                    }
        ...
        console.log(`[API CALLS GET] Found ${agentCalls.length} calls for agent ${agentId}`);
        ...
        console.error(`[API CALLS GET] Database error fetching calls for agent ${agentId} (user ${userId}):`, error);
