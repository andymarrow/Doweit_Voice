...
export async function POST(req, { params }) {
	try {
		...
		console.log(
			"Creating temporary VAPI assistant with payload:",
		...
		console.log(
			`Successfully created temporary assistant with ID: ${newAssistantId}`,
		);
		...
		if (!startCallResponse.ok) {
			...
			const errorMessage =
				...
				callData.error ||
				"Failed to initiate call with VAPI.";
			return NextResponse.json({ error: errorMessage }, { status: 500 });
		}

		return NextResponse.json(
			{ success: true, callId: callData.id },
			{ status: 200 },
		);
	} catch (error) {
