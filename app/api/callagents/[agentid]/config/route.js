...
export async function GET(req, { params }) {
	...
	if (!userId) {
		console.log(
			`[API CONFIG GET] Unauthorized: No userId for agent ${agentId}`,
		);
	...
	try {
		console.log(
			`[API CONFIG GET] Fetching config for agent ${agentId} (user ${userId})`,
		);
		...
		if (!agent) {
			console.warn(
				`[API CONFIG GET] Agent ${agentId} not found for user ${userId}`,
			);
			return NextResponse.json({ error: "Agent not found" }, { status: 404 });
		}

		console.log(
			`[API CONFIG GET] Successfully fetched config for agent ${agentId}`,
		);
		// Return the fetched agent object (or a subset of its fields)
		return NextResponse.json(agent, { status: 200 });
	} catch (error) {
		console.error(
			`[API CONFIG GET] Database error fetching config for agent ${agentId} (user ${userId}):`,
			error,
		);
...
export async function PATCH(req, { params }) {
	...
	if (!userId) {
		console.log(
			`[API CONFIG PATCH] Unauthorized: No userId for agent ${agentId}`,
		);
		return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
	}
	...
	try {
		...
		console.log(
			`[API CONFIG PATCH] Received update request for agent ${agentId} (user ${userId}). Updates:`,
			updates,
		);
		...
		for (const field in updates) {
			if (ALLOWED_UPDATE_FIELDS.includes(field)) {
				...
				if (field === "customVocabulary" && !Array.isArray(updates[field])) {
					console.warn(
						`[API CONFIG PATCH] Invalid type for customVocabulary: expected array`,
					);
					continue; // Skip this field or return error
				}
				if (field === "voiceConfig" && typeof updates[field] !== "object") {
					console.warn(
						`[API CONFIG PATCH] Invalid type for voiceConfig: expected object`,
					);
					continue; // Skip
				}
				if (field === "callConfig" && typeof updates[field] !== "object") {
					console.warn(
						`[API CONFIG PATCH] Invalid type for callConfig: expected object`,
					);
					continue; // Skip
				}
				...
				if (
					...
					console.warn(
						`[API CONFIG PATCH] Invalid type for knowledgeBaseId: expected number or null`,
					);
					continue; // Skip
				}
				...
				if (field === "useFillerWords" && typeof updates[field] !== "boolean") {
					console.warn(
						`[API CONFIG PATCH] Invalid type for useFillerWords: expected boolean`,
					);
					continue; // Skip
				}
				...
				console.warn(
					`[API CONFIG PATCH] Attempted to update disallowed field: ${field}`,
				);
		...
		if (Object.keys(updateData).length === 0) {
			console.warn(
				`[API CONFIG PATCH] No valid update fields provided for agent ${agentId}`,
			);
			return NextResponse.json(
		...
		console.log(
			`[API CONFIG PATCH] Applying updates for agent ${agentId}:`,
			updateData,
		);
		...
		if (!updatedAgents || updatedAgents.length === 0) {
			...
			console.warn(
				`[API CONFIG PATCH] Update failed: Agent ${agentId} not found or not owned by user ${userId}`,
			);
			return NextResponse.json(
		...
		console.error(
			`[API CONFIG PATCH] Unexpected API Error during agent update for agent ${agentId} (user ${userId}):`,
			error,
		);
		return NextResponse.json(
			{ error: "Internal server error" },
