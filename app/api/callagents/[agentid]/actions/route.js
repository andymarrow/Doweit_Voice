...
export async function POST(req, { params }) {
	...
	if (!userId) {
		console.log(
			`[API AGENT ACTIONS POST] Unauthorized: No userId for agent ${agentId}`,
		);
		return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
	}
	...
	try {
		...
		console.log(
			`[API AGENT ACTIONS POST] Received request to add actions to agent ${agentId} (user ${userId})`,
			{ timing, rawActionIds },
		);
		...
		console.log(
			`[API AGENT ACTIONS POST] Processed unique actionIds:`,
			actionIds,
		);
		...
		if (!timing || !["before", "during", "after"].includes(timing)) {
			console.warn(
				`[API AGENT ACTIONS POST] Invalid or missing timing provided: ${timing}`,
			);
			return NextResponse.json(
				{ error: "Invalid or missing timing provided" },
				{ status: 400 },
			);
		}
		if (actionIds.length === 0) {
			...
			return NextResponse.json(
				{ message: "No actions provided to add" },
				{ status: 200 },
			); // Or 400, depending on desired behavior
		}
		...
		if (!agent) {
			console.warn(
				`[API AGENT ACTIONS POST] Agent ${agentId} not found or not owned by user ${userId}`,
			);
			return NextResponse.json(
				{ error: "Agent not found or not authorized" },
				{ status: 404 },
		...
		if (requestedGlobalActions.length !== actionIds.length) {
			...
			console.warn(
				`[API AGENT ACTIONS POST] Attempted to add disallowed or non-existent global actions. Disallowed IDs: ${disallowedIds.join(", ")} for user ${userId}`,
			);
			// Return 403 Forbidden if any requested unique ID was not found/allowed
			return NextResponse.json(
				{
					error: `One or more selected actions are not available or you do not have permission to add them. Disallowed IDs: ${disallowedIds.join(", ")}`,
				},
				{ status: 403 },
			);
		}
		...
		let maxOrder = existingActionsInTiming.reduce(
		...
		console.log(
			`[API AGENT ACTIONS POST] Preparing to insert ${valuesToInsert.length} agentActions.`,
		);
		...
		const insertedAgentActions = await db
			.insert(agentActions)
		...
		console.log(
			`[API AGENT ACTIONS POST] Successfully inserted ${insertedAgentActions.length} new agent actions for agent ${agentId}`,
		);
		...
		if (insertedAgentActions.length > 0) {
			...
			console.log(
				`[API AGENT ACTIONS POST] No new actions inserted (likely duplicates).`,
			);
			return NextResponse.json([], { status: 200 }); // Return empty array, status 200 OK
		}
	} catch (error) {
		console.error(
			`[API AGENT ACTIONS POST] Unexpected API Error during agent action addition for agent ${agentId} (user ${userId}):`,
			error,
		);
		...
		if (error.message.includes("action_creator_name_unq")) {
			// Check for the global actions unique index name (unlikely here, this is for *creating* global actions)
			console.warn(
				`[API AGENT ACTIONS POST] Potential unique constraint violation during lookup/insert.`,
			);
