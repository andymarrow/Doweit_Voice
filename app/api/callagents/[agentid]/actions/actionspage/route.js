...
export async function GET(req) {
    ...
    try {
        console.log(`[API ACTIONS GET] Fetching available actions for user ${userId}`);
        ...
        console.log(`[API ACTIONS GET] Found ${availableActions.length} available actions for user ${userId}`);

        return NextResponse.json(availableActions, { status: 200 });

    } catch (error) {
        console.error(`[API ACTIONS GET] Database error fetching available actions for user ${userId}:`, error);
